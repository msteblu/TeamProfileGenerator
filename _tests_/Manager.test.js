...
describe("Manager class", () => {
  describe("Initialization", () => {
    it("Manager object was created correctly", () => {
      ...
      const obj = new Manager(id, name, email);
      // Assert
    ...
    it("Should throw an error if not provided text for id", () => {
      ...
      const cb = () => new Manager("", "Name", "name@email.com");
      // Assert
    ...
    it("Should throw an error if not provided text for name", () => {
      ...
      const cb = () => new Manager("1", "", "name@email.com");
      // Assert
      expect(cb).toThrowError(err);
    });
    ...
    it("Should throw an error if email address is not formatted correctly", () => {
      ...
      const cb = () => new Manager("1", "Name", "email");
      // Assert
      expect(cb).toThrowError(err);
    });        
  });
  ...
  describe("Get ID", () => {
    ...
    // Arrange
    const obj = new Manager("1", "Name", "name@email.com");
    // Act
    const id = obj.getId();
    // Assert
